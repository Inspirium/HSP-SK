...
class HomeController extends Controller {
	public function home() {
		$total = \DB::table('propositions')->count();
		$approval = \DB::table('propositions')->where('status', 'requested')->count();
		$files = \DB::table('fileables')->where('fileable_type', 'Inspirium\BookProposition\Models\BookProposition')->get(['fileable_id', 'type']);
		$latest = [];
		$order = [
